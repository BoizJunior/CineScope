...
export default function Hero({ movies, onPlayClick }: HeroProps) {
  ...
  return (
    <div 
      className="relative w-full h-[75vh] md:h-[80vh] bg-[#141414] overflow-hidden mb-0"
      onTouchStart={handleTouchStart}
      onTouchEnd={handleTouchEnd}
    >
      {/* LAYER 1: Background Image (Full Width) */}
      <div className="absolute inset-0 w-full h-full z-0">
        <div className="relative w-full h-full md:flex md:justify-end">
      ...
      {/* LAYER 2: Content (Constrained Width) */}
      <div className="relative z-10 w-full h-full max-w-[1920px] mx-auto px-4 md:px-12 flex flex-col justify-end pb-12 md:pb-10">
        <div className="w-full max-w-3xl">
          
          {/* Title */}
          <h1 className="text-4xl md:text-6xl 2xl:text-7xl font-bold text-white mb-2 md:mb-4 uppercase leading-tight drop-shadow-xl text-center md:text-left">
          ...
          {/* Genres */}
          {movieDetails?.genres && (
            <div className="text-gray-300 text-xs md:text-sm font-medium mb-4 md:mb-6 text-center md:text-left">
              {movieDetails.genres.slice(0, 3).map((g) => g.name).join(' • ')}
            </div>
          )}

          {/* Description */}
          <p className="text-gray-300 text-sm md:text-lg 2xl:text-xl mb-6 md:mb-8 line-clamp-3 leading-relaxed text-center md:text-left">
